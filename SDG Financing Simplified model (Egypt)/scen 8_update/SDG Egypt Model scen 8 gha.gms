$title SDG Egypt Model scen 8 gha
$stitle Last Updated: August 28, 2022
...
tmid(t) = yes$(ord(t) ge 2 and ord(t) le card(t)-1);
...
*Define scenarios for interest rate and debt limit
Set     scen /lim,lowr,highdebt/;

* Parameter Declaration
***********************

Parameter   qs(scen,t)     Total GDP by scenario and year
            ...
            ks(scen,t)      Capital stock by sector scenario and year
            edugdps(scen,t)      Output of education sector by scenario and year
            hlgdps(scen,t)    Output of health sector by scenario and year
            invs(scen,t)    Investment by sector scenario and year
...
parameter bk        bk coefficient on business capital stock (plant and equipment) in the production function
          ...
          hwratio   Healthcare worker density per capita
          hcaccum   Human capital accumulation rate by year
          fertsh
          pubgdpsh
          lowerbound
          continuation(a, conti);

$CALL GDXXRW .\input\paraminput\paraminput_gha.xlsx output=.\input\auxiliary\paraminput_gha.gdx Index=Index!a1 trace=3
$GDXIN .\input\auxiliary\paraminput_gha.gdx
$LOAD bk bkf ben k0 kf0  costff costre dep  discr kff0 kre0 aff tfp1 phi stratio hwratio hcaccum fertsh pubgdpsh lowerbound continuation
$GDXIN
...
display bk, bkf, ben, blab, k0, kf0,  costff, costre, dep, discr, kff0, kre0, aff, tfp1, phi, stratio, hwratio, hcaccum, fertsh, pubgdpsh, lowerbound;
...
$CALL GDXXRW .\input\auxiliary\datainput_gha.xlsx output=.\input\auxiliary\datainput_gha.gdx Index=index!a1 trace=3
$GDXIN .\input\auxiliary\datainput_gha.gdx
...
Equations
                    ...
                    eq73(t)        Consumption per capita grows over time
;
...
eq73(t).. conpc(t) =g= conpc(t-1)*1.01;
...
Equations 
                    ...
                    invtotal(t)     Total investment back in the economy
                    ...
                    endconpclim     Make the output looks nicer
                    nodebtbyend(t)  No debt outstanding by the end
                    posnxbyend(t)   Positive nx by end (no more borrowing beyond)
                    ;
                    
output(t)..           q(t) =e= tfp1*en(t)**ben*kf(t)**bkf*k(t)**bk*efflabtot(t)**blab;
outputpc(t)..         qpc(t) =e= q(t)/poptot(t);
ewage(e,t)..          wage(e,t) =e= (blab*q(t)/(lowerbound+efflabtot(t)))*exp(hcaccum*(years(e)-9));
...
kend(tend)..          inv(tend) =g= k(tend)*dep;
kffend(tend)..        invff(tend) =g= kff(tend)*dep;
kreend(tend)..        invre(tend) =g= kre(tend)*dep;
kfend(tend)..         invf(tend) =g= kf(tend)*dep;
ktotal(t)..           ktot(t) =e= k(t) + kf(t) + kff(t) + kre(t);
invtotal(t)..         invtot(t) =e= invff(t) + invre(t) + invf(t) + inv(t);
consumetrpc(t)..      conpc(t) =e= con(t)/poptot(t);
endconpclim..         invtot("2050") =g= invtot("2049");

* Assume half with at least a bachelor's and the rest with a high-school diploma
* perhaps include student share in educationuc
educationuc(t)..      edunitcost(t) =e= (0.5*wage("ts",t)+0.5*wage("us",t))/stratio/qpc(t);
healthuc(t)..         hlunitcost(t) =e= (0.5*wage("ts",t)+0.5*wage("us",t))*hwratio/qpc(t);

education(t)..        edcost(t) =e= edunitcost(t)*schooltot(t);
health(t)..           hlcost(t) =e= hlunitcost(t)*poptot(t);
govcost(t)..          gcost(t) =e= pubgdp(t)*q(t);
...
utility..             util =e= sum(t,disc(t)* ut(t)) + (1/(discr))*sum(tend, disc(tend)*(sum(t, (ord(t))*ut(t))/sum(t, (ord(t)))));
...
enktots(scen,t)=enktot.L(t);
debts(scen,t)=debt.L(t);
kffs(scen,t) = kff.L(t);
kres(scen,t) = kre.L(t);
kfs(scen,t) = kf.L(t);
...
efflabtots(scen,t) = efflabtot.L(t);
pas(scen,t) = pa.L(t);
lsas(scen,t) = lsa.L(t);
usas(scen,t) = usa.L(t);
tsas(scen,t) = tsa.L(t);
invs(scen,t) = inv.L(t);
edugdps(scen,t)=edugdp.L(t);
hlgdps(scen,t)=hlgdp.L(t););

display pas, lsas, usas, tsas, ktots, kffs, kres,invs, qs, qpcs,efflabtots,conpcs,enktots, poptots,debts, debtgdps, ffs;

set outputvarlist
/"Primary age students",
"Lower secondary age students",
...
"Energy capital as a percentage of total capital"
"Total Population",
...
parameter outputvar(outputvarlist,scen,t);
        outputvar("Primary age students",scen,t) = pas(scen,t);
        outputvar("Lower secondary age students",scen,t) = lsas(scen,t);
        outputvar("Upper secondary age students",scen,t) = usas(scen,t);
        outputvar("Tertiary age students",scen,t) = tsas(scen,t);
        outputvar("Total Capital",scen,t) = ktots(scen,t);
        outputvar("Energy Capital - Fossil Fuel",scen,t) = kffs(scen,t);
        outputvar("Energy Capital - Renewables",scen,t) = kres(scen,t);
        outputvar("Investment",scen,t)=invs(scen,t);
        outputvar("Output",scen,t)=qs(scen,t);
        outputvar("Output per Capita",scen,t) = qpcs(scen,t);
        outputvar("Consumption per Capita",scen,t) = conpcs(scen,t);
        outputvar("Energy capital as a percentage of total capital",scen,t) = enktots(scen,t);
        outputvar("Total Population",scen,t) = poptots(scen,t);
        outputvar("Total Efficient Labor Unit",scen,t) = efflabtots(scen,t);    
        outputvar("Debt Level",scen,t) = debts(scen,t);
        outputvar("Debt to GDP",scen,t) = debtgdps(scen,t);
        outputvar("Fossil Fuel Cost",scen,t) = ffs(scen,t);
        
Execute_Unload '.\input\auxiliary\modeloutput_gha.gdx', outputvar;

$onEcho > .\input\auxiliary\outputsetting_gha.txt
par=outputvar
$offEcho
Execute 'GDXXRW.EXE .\input\auxiliary\modeloutput_gha.gdx output=.\output\modeloutput_gha.xlsx @.\input\auxiliary\outputsetting_gha.txt';