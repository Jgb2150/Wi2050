$title SDG Egypt Model v5 moz
$stitle Last Updated: August 13, 2022
...
display bk, bkf, ben, k0, kf0, debtqlim, costff, costre, dep, r, discr, kff0, kre0, aff, tfp1, phi, fertsh, gcostsh, pubgdpsh, lowerbound;
...
positive variables    lfe(e,t)              Labor force as a function of education and year
                      ...
                      condebt(t)            Debt used to finance consumption (more expensive because not used for cash flow generating projects)
                      condebtbal(t)         Consumption debt stock
                      edcoef(t)             Level of education spending (% of optimal)
                      hlcoef(t)             Level of education spending (% of optimal)
...
Equations
                    ...
                    eq73(t)        People aren't willing to fasten their bellies too much
                    eq74(t)        Education coefficient should be between 0 and 1
                    eq75(t)        Healthcare coefficient should be between 0 and 1
                    eq76(t)        Last year must pay fully for education
                    eq77(t)        Last year must pay fully for healthcare
;

eq1(as+1,g,t+1)..  s(as+1,g,t+1) =e= edcoef(t+1)*cont(as,t)*s(as,g,t);
eq2(g,t+1)..  s("6",g,t+1) =e= edcoef(t+1)*pop("5",g,t);
...
eq73(t).. conpc(t) =g= conpc(t-1);
eq74(t).. edcoef(t) =l= 1;
eq75(t).. hlcoef(t) =l= 1;
eq76(tend).. edcoef(tend) =e= 1;
eq77(tend).. hlcoef(tend) =e= 1;
...
Equations 
                    ...
                    totcondebt(t)   Total consumption debt
                    initcondebtbal(t)  Initial stock of consumption debt is 0
*initcondebt(t)  Borrow enough in the beginning
...
totcondebt(t+1)..     condebtbal(t+1) =e= 1.1*(1+r)*condebtbal(t);
initcondebtbal(tstart).. condebtbal(tstart) =e= 0;
*initcondebt(tstart).. condebt(tstart) =g= max(0, -(q(tstart) - dep*(k(tstart)+kf(tstart)+kre(tstart)+kff(tstart)) - (edcostgdp(tstart)+hlcostgdp(tstart)+pubgdp(tstart))*q(tstart) - ff(tstart))+qpc(tstart)*0.2)
debttogdp(t)..        debtgdp(t) =e= (debt(t)+condebtbal(t))/(q(t)+lowerbound);
debtlimit(t)..        debtgdp(t) =l= debtqlim;
conlim(t)..           con(t) =l= q(t) + condebt(t) - dep*(k(t)+kf(t)+kre(t)+kff(t)) - r*(debt(t)+condebtbal(t)) - (edcostgdp(t)+hlcostgdp(t)+pubgdp(t))*q(t) - ff(t);
conend(tend)..        con(tend) =e= q(tend) - cinv(tend) - cinvf(tend) - cinvff(tend) - cinvre(tend) - r*(debt(tend)+condebtbal(tend)) - (pubgdp(tend)+edcostgdp(tend)+hlcostgdp(tend))*q(tend)-ff(tend);
...
educationuc(t)..      edunitcost(t) =e= edcoef(t)*2*((1/10)*wage("ts",t)+(1/10)*wage("us",t))/qpc(t);
healthuc(t)..         hlunitcost(t) =e= hlcoef(t)*2*((1/50)*wage("ts",t)+(1/50)*wage("us",t))/qpc(t);
education(t)..        edcost(t) =e= edunitcost(t)*schooltot(t);
health(t)..           hlcost(t) =e= hlunitcost(t)*poptot(t);
govcost(t)..          gcost(t) =e= gcostsh*q(t);
edugdp(t)..           edcostgdp(t) =e= edcost(t)/(q(t)+lowerbound);
hlgdp(t)..            hlcostgdp(t) =e= hlcost(t)/(q(t)+lowerbound);
poutlay(t)..          outlaygdp(t) =e= edcostgdp(t) + hlcostgdp(t) + pubgdp(t) + invfgdp(t);
pubadgdp(t)..         pubgdp(t) =e= pubgdpsh;
infgdp(t)..           invfgdp(t) =e= cinvf(t)/(q(t)+lowerbound);
utilt(t)..            ut(t) =e= log(conpc(t)*edcoef(t)*hlcoef(t)+lowerbound);
*utilt(t)..            ut(t) =e= log(conpc(t)) - 1000*(1-edcoef(t)) - 1000*(1-hlcoef(t));
...
display q.L, qpc.L, conpc.L, poptot.L, efflabtot.L, kff.L, kre.L, ktot.L, debt.L, debtgdp.L, ff.L;
